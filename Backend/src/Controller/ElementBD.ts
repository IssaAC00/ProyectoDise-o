...
export async function ElementoXArea(req: Request, res: Response) {
    try{
        //const id = req.params.postId;
        const conn = await MySQL.getInstance().getConnect();
        const elements = await conn.query('select Area.idArea, Element.idElement, Element.description, Element.ubication from Element INNER JOIN Area on Element.areaID = Area.idArea');
        res.json(elements);
    }catch(e){
        console.log(e);
    }
    //select * from Element where Element.areaID= ?, [id]; 
