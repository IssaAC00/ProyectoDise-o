import React from "react";
import {Button,  Container, Navbar as NavbarBs } from "react-bootstrap"
import './TopBar.css'



export default function TopBarLogin(){

    return (
        <NavbarBs sticky="top" className='topbarContainer'>
        <Container >
        <label className="Titulo">Teatro Nacional Gestión de Patrimonios</label>
        <svg xmlns="http://www.w3.org/2000/svg" width="45" height="20" fill="white" className="svg"  >
        <path d="M8.354 1.146a.5.5 0 0 0-.708 0l-6 6A.5.5 0 0 0 1.5 7.5v7a.5.5 0 0 0 .5.5h4.5a.5.5 0 0 0 
        .5-.5v-4h2v4a.5.5 0 0 0 .5.5H14a.5.5 0 0 0 .5-.5v-7a.5.5 0 0 0-.146-.354L13 5.793V2.5a.5.5 0 0
         0-.5-.5h-1a.5.5 0 0 0-.5.5v1.293L8.354 1.146zM2.5 14V7.707l5.5-5.5 5.5 5.5V14H10v-4a.5.5 0 0 0-.5-.5h-3a.5.5 0 0 0-.5.5v4H2.5z"/>
        </svg>


        </Container>
      </NavbarBs>
    );
      
    }
    